...
#include <stdio.h>
#include <stdlib.h> // 标准库头文件，提供了一些常用的函数
#include <stdlib.h>
...
#include <sys/types.h>
#include <sys/wait.h> // 提供了等待子进程结束的相关函数
#include <sys/wait.h>
#include <unistd.h> // 提供了POSIX操作系统API的访问功能
#include <unistd.h>
...
void eval(char *cmdline) {
  ...
  if (builtin_cmd(argv)) {
    ...
    if (!bg) {
      addjob(jobs, job_pid, FG, buf);
      sigprocmask(SIG_SETMASK, &prev_mask, NULL);
      waitfg(job_pid);
    } else {
      addjob(jobs, job_pid, BG, buf);
      sigprocmask(SIG_SETMASK, &prev_mask, NULL);
      printf("[%d] (%d) %s", pid2jid(job_pid), job_pid, cmdline);
    }
  }
}
/*
